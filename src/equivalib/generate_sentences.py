...
from equivalib import Sentence, GeneratorMyType, MaxgreedyMyType, GreedyMyType
import equivalib


def generate_suffixes(t: GeneratorMyType, ret: List[Sentence]) -> Generator[Sentence, None, None]:
    for prefix in ret:
        if isinstance(t, MaxgreedyMyType):
            yield from list(equivalib.extend_sentence_maxgreedily(prefix, t.x))
        elif isinstance(t, GreedyMyType):
