...
def test_generate_accepts_address_bounded_ints_inside_unnamed_tuple():
    generate = core_attr("generate")
    tree = Tuple[int, int]
    a = ref(0)
    b = ref(1)
...
def test_generate_accepts_address_bounded_ints_inside_unnamed_tuple_three_items():
    generate = core_attr("generate")
...
def test_structural_eq_distinguishes_bool_from_int():
    ...
    Python's ``True == 1`` would make ``Eq(ref("X"), IntegerConstant(1))``
...
def test_bool_reference_as_constraint_direct():
    """ref("X") used directly as constraint must work (P1 regression)."""
    generate = core_attr("generate")
    Name = core_attr("Name")
