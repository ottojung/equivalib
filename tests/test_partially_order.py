...
import pytest
from equivalib.all import partially_order


def test_empty_list():
    assert list(partially_order([], lambda x: [])) == []


def test_singleton_list():
    assert list(partially_order([1], lambda x: [])) == [[1]]
...
def test_incomparable_elements():
    elements = [1, 2, 3, 4]
    before_relation: Callable[[int], Iterable[int]] = lambda x: []
...
def test_complex_partial_order():
    ...
    before_relation: Mapping[int, List[int]] = {
        1: [],
        2: [],
        3: [1, 2],
        4: [2],
...
def test_larger_set_with_duplicates():
    ...
    assert list(partially_order(elements, before_relation)) == [[1], [2, 2], [3], [4, 4, 4], [5]]


def test_disconnected_graph():
    elements = ['a', 'b', 'c', 'd', 'e', 'f']
    before_relation: Mapping[str, List[str]] = {
        'a': [],
        'b': [],
        'c': [],
        'd': [],
